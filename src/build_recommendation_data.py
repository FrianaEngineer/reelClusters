...
neighbors (movie_edges), color/b&w (film_color.csv), outbound links
(criterion_film_links.csv / criterion_channel_links.csv), and each film's real
original-language code (data/film_language.csv, see
src/fetch_film_languages.py -- sourced from TMDB, keyed by imdb_tconst).

No plot summaries or posters are invented; language is the one field sourced
from an external API rather than this project's own IMDb/Criterion match
data, because no structured per-film language field exists anywhere else in
the pipeline (see fetch_film_languages.py docstring). A film TMDB has no
match for gets `language: null` rather than a guess.
...
def load_language():
    """imdb_tconst -> ISO 639-1 original-language code, from TMDB (see
    fetch_film_languages.py). Films TMDB has no match for, or that failed to
    fetch, are simply absent from the map -- callers get None, not a guess."""
    path = DATA_DIR / "film_language.csv"
    if not path.exists():
        return {}
    df = pd.read_csv(path)
    df = df[(df["status"] == "ok") & df["original_language"].notna()]
    return dict(zip(df["imdb_tconst"], df["original_language"]))
...
def build_records():
    ...
    for row in basic.itertuples(index=False):
        ...
        records.append({
            ...
            "language": language_map.get(tconst),
            "year": year,
            "runtime": runtime,
...
def main():
    ...
    js = (
        ...
        "//   t, title, director, country, englishSpeaking, language, year,\n"
        "//   runtime, genres[], cluster, clusterName, color, colorLabel,\n"
        "//   degree, neighbors[{t, shared}], criterionUrl, channelUrl\n"
